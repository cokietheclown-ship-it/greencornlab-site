...
`manifest.webmanifest` と `sw.js` を置いてあるので、HTTPS で配れば
そのままホーム画面に追加できるアプリになる。

- iOS / Safari — 共有ボタン →「ホーム画面に追加」（自動の案内バナーも出る）
- Android / Chrome — アドレスバーの「インストール」、または画面上部の案内バナー

追加後は全画面（`display: standalone`）で開き、Service Worker が画像・国境データを
まとめてキャッシュするので **機内モードでも遊べる**。アセットを差し替えたら
`sw.js` の `CACHE` の版番号を上げること。上げないと古いものが返り続ける。

実機向けに、`deviceMemory` が 4GB 以下か小さいタッチ画面の端末では
テクスチャを 2048px、球の分割を 160×80、`devicePixelRatio` を 1.75 に落としている。
